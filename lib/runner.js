...
import { startResourceMonitor } from "./resources.js";
...
export async function runGuard(options) {
  ...
  const settleSeconds = options.settleSeconds ?? (options.mode === "post-upgrade" ? 120 : 0);
  if (settleSeconds > 0) {
    commands.gateway_settle = await waitForGatewaySettle(options.openclaw, {
      settleSeconds,
  ...
  // Fallback: if status command timed out or failed, use gateway_probe data as a partial substitute
  // so that evaluateStatus() and evaluateBaseline() can still run meaningful checks.
  if (!commands.status?.ok && commands.gateway_probe?.json) {
    const probe = commands.gateway_probe.json;
    commands.status = {
      id: "status",
      args: ["status", "--json"],
      required: true,
      ok: false,
      timedOut: commands.status?.timedOut ?? false,
      exitCode: commands.status?.exitCode ?? null,
      durationMs: commands.status?.durationMs ?? 0,
      stdout: commands.status?.stdout ?? "",
      stderr: commands.status?.stderr ?? "",
      parseError: commands.status?.parseError ?? "status command was unavailable; gateway_probe used as fallback",
      json: {
        runtimeVersion: probe.self?.version ?? null,
        gateway: {
          reachable: probe.ok === true,
          url: probe.network?.localLoopbackUrl ?? null,
          misconfigured: false,
        },
        ...
        taskAudit: null,
        _fallback: true,
        _fallbackReason: "gateway_probe used because openclaw status --json was unavailable",
      },
    };
  }
  ...
  const checks = evaluate({ commands, baseline, mode: options.mode, resources });
...
async function waitForGatewaySettle(bin, { settleSeconds, timeoutSeconds, progress }) {
  const command = {
    id: "gateway_settle",
    label: "Wait for gateway readiness after upgrade",
    args: ["gateway", "probe", "--json"],
    json: true,
    required: false,
  };
  const startedAt = Date.now();
  const deadline = startedAt + settleSeconds * 1000;
  const attemptTimeout = Math.max(1, Math.min(timeoutSeconds, 15));
  let attempt = 0;
  let lastResult;

  progress({ type: "phase", message: `Waiting up to ${settleSeconds}s for gateway readiness before post-upgrade checks` });
  do {
    attempt += 1;
    progress({
      type: "command-start",
    ...
    lastResult = await runCommandOnce(bin, command, attemptTimeout, attempt);
    const ready = lastResult.json?.ok === true;
    progress({ type: "command-end", command, result: lastResult, retrying: !ready && Date.now() < deadline });
