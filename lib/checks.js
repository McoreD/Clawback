...
export function evaluate({ commands, baseline, mode, resources }) {
  ...
  for (const command of Object.values(commands)) {
    if (command.required && !command.ok) {
      // Downgrade status timeouts to warnings since gateway_probe data is used as fallback
      if (command.id === "status" && command.timedOut) {
        add("warning", `command.${command.id}`, `Required command timed out (gateway_probe used as fallback): openclaw ${command.args.join(" ")}`, commandSummary(command));
      } else {
  ...
  evaluateContainerFidelity({ mode, status, health, add });
  evaluateBaseline({ baseline, status, health, mode, commands, resources, add });

  return checks;
}

function evaluateContainerFidelity({ mode, status, health, add }) {
  if (mode !== "container-rehearsal") return;
  const limitations = [
    "user systemd gateway service is not exercised inside the container",
    "live channel auth stores and device state are intentionally omitted by the sanitized fixture",
    "absolute host workspace paths and external project directories are not mounted by default",
    "durable task history, locks, logs, media, memory, and runtime caches are omitted by default",
  ];
  const evidence = {
    gatewayServiceInstalled: status?.gatewayService?.installed === true,
    configuredChannels: Object.entries(health?.channels || {})
      .filter(([, channel]) => channel.configured)
      .map(([name]) => name),
    linkedChannels: Object.entries(health?.channels || {})
      .filter(([, channel]) => channel.linked === true || Object.values(channel.accounts || {}).some((account) => account.linked === true))
      .map(([name]) => name),
  };
  add(
    "warning",
    "container.fidelity.host_replica",
    "Container rehearsal is a sanitized compatibility smoke test, not a full replica of the live host",
